...
output_file = "../../analysis/{}/{}_HistogramPlot.pdf".format(run_number, run_number)

# reading in data
df = pd.read_csv(input_file, delimiter='\t', names=['steps', 'activated', 'silenced', 'bivalent', 'total'], header=None)
...
# fig.suptitle('Nucleosome state amount (total: 40 nucleosomes)')

# fig.text(0.5, 0.04, 'step number', ha='center')
...
activated_gs = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec = outer_gs[0,0], width_ratios=[1,1], wspace=0.02)
silenced_gs = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec = outer_gs[0,1], width_ratios=[1,1], wspace=0.02)
...
# ax13.set_ylim([0,40])
# ax13.ticklabel_format(useOffset=False, style='plain')
